...
// Best Fitting Model for Complex Span - fixed f model
  ...
  matrix[J,N] subj_pars =  (
    ...
    * theta
    ...
    for(j in 1:Con) {
      ...
      acts_IIP[j + (i-1)*Con] = scale_b +  ((1+subj_pars[3,i]*Freetime[j])* subj_pars[1,i]) + subj_pars[2,i]; // Item in Position                      
      acts_IOP[j + (i-1)*Con] = scale_b + subj_pars[2,i];        // Item in Other Position
      acts_DIP[j + (i-1)*Con] = scale_b + 0.5*((exp(-subj_pars[4,i]*Freetime[j])*subj_pars[1,i]) + subj_pars[2,i]); // Distractor in Position
      acts_DIOP[j + (i-1)*Con] = scale_b + 0.5*subj_pars[2,i]; // Distractor in other Position
